...
function SettingsRow({
  ...
  return (
    ...
    >
      ...
      {right && (
        <span className={`text-xs font-medium transition-colors ${isClient ? "text-zinc-600 group-hover:text-zinc-400" : "text-zinc-400 group-hover:text-zinc-600"}`}>
          {right}
        </span>
      )}
    </div>
    ...
    <div className={`min-h-screen font-sans ${isClient ? "bg-black text-white" : "bg-[#FAF9F7] text-[#2A2520]"}`}>
      ...
      <main className="px-6 pt-12 pb-24 max-w-2xl mx-auto">
        <header className="mb-10">
          <p className={`text-[10px] uppercase tracking-[0.2em] font-bold mb-2 ${isClient ? "text-zinc-600" : "text-zinc-400"}`}>
            Preference Control
          </p>
          <h1 className={`text-4xl font-bold ${isClient ? "bg-linear-to-b from-white to-zinc-500 bg-clip-text text-transparent" : "text-zinc-900"}`}>
            Settings
          </h1>
        </header>
        ...
        <section>
              ...
              <SettingsRow
                ...
                right={
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-[10px] underline underline-offset-4 font-bold border-l border-white/10 pl-3 ml-1 uppercase tracking-tighter ${copied ? "text-emerald-400" : ""}`}>
                      {copied ? "Copied ✓" : "Copy"}
                    </span>
                  </div>
                }
          ...
          <SettingsGroup
            isClient={isClient}
            rows={[
              <SettingsRow key="signout" label="Sign out of Vekko" danger onClick={handleSignOut} isClient={isClient} />,
            ]}
          />
        </section>
      </main>
