...
export function WalletBalanceCard({
  ...
  const cardBg = isClient ? "bg-zinc-900/40 border-white/5 backdrop-blur-md" : "bg-white border-zinc-200/60";
      ...
      <section>
        ...
        <div className={`rounded-3xl border overflow-hidden divide-y ${isClient ? "bg-zinc-900/40 border-white/5 divide-white/5" : "bg-white border-zinc-200/60 divide-zinc-100"}`}>
          {transactions.map((txn) => (
            <div key={txn.id} className={`flex items-center justify-between p-6 transition-colors ${isClient ? "hover:bg-white/2" : "hover:bg-zinc-50"}`}>
              <div className="flex flex-col gap-1">
                <span className={`text-sm font-semibold ${isClient ? "text-zinc-200" : "text-zinc-900"}`}>
                  {txn.description}
                </span>
