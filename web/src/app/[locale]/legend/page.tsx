...
import InsiderNav from "@/app/[locale]/dashboard/_components/InsiderNav";
import { LegendForm } from "./_components/LegendForm";
...
export default async function LegendPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("legend");

  const { userId, role } = await getCurrentUser();
  if (role !== "insider") redirect(`/${locale}/dashboard`);
        ...
        <header className="mb-10">
          ...
          {legend.ok && (
            <p className="text-sm text-zinc-400 mt-3 leading-relaxed">
              {text ? t("whatClientsSee") : t("whatClientsSeeEmpty")}
            </p>
          )}
        </header>
