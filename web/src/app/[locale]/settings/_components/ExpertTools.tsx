...
import { createApiKey, revokeApiKey, type ApiKeyMeta } from "@/actions/auth";
...
export function ExpertTools({
  ...
  const t = useTranslations("settings.expertTools");
  ...
  function handleCreate() {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) {
        setError(tErrors(messageFor("identity.apiKeys", res.error.code)));
        return;
      }
            ...
            <ul className="flex flex-col gap-2">
              {active.map((k) => (
                <li key={k.id} className="flex items-center justify-between">
                  ...
                  >
                    {maskedRow(k.prefix)}
                    {k.name ? <span className="opacity-50"> · {k.name}</span> : null}
                  </span>
                  <button
                    type="button"
