...
export function CompleteOrderButton({ orderId, status }: { orderId: string; status: OrderStatus }) {
  const t = useTranslations("completeOrder");
  ...
  async function handleComplete() {
    ...
    if (!res.ok) {
      setError(tErrors(messageFor("interaction.order", res.error.code)));
    }
    setLoading(false);
  }
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      >
        {loading ? t("completing") : t("markCompleted")}
      </button>
      <p className="text-[10px] text-zinc-600 max-w-[16rem] text-right">{t("closesOrder")}</p>
