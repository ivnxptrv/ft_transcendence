...
| Get responses for order | `GET`  | `/orders/:id/responses`                         | —                  | `InsightCard[]` (insights hidden if `isUnlocked: false`)   |
| Unlock a response       | `POST` | `/orders/:orderId/responses/:responseId/unlock` | —                  | `{ insiderInsight: string }` — deducts `price` from wallet |

**Frontend files:** `src/app/dashboard/_components/NewOrderButton.tsx`, `src/app/orders/[id]/page.tsx`, `src/app/orders/_components/InsightCardView.tsx`
...
| Submit insight   | `POST` | `/matches/:id/respond` | `{ text, price }` | `{ insight: Insight, match: Match }` — sets `match.status = "responded"` |
| Skip match       | `POST` | `/matches/:id/skip`    | —                 | `{ match: Match }` — removes from active list                            |

**Frontend files:** `src/app/matches/[id]/page.tsx`, `src/app/matches/_components/MatchInsightForm.tsx`
...
type Order = {
  ...
  title;
  query;
  status: "pending" | "has_responses" | "completed";
  createdAt;
  insightCount;
};
type InsightCard = {
...
type Match = {
  ...
  query;
  insiderId;
  status: "new" | "responded" | "purchased" | "rated";
  matchScore;
  receivedAt;
  insight?: Insight;
};
...
| `getInsightsForOrder(orderId)`    | `GET /orders/:orderId/responses` |
